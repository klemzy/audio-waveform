...
import com.github.derlio.waveform.util.ConversionUtils;
...
{
	public static final String TAG = SimpleWaveformView.class.getSimpleName();
	...
	private void initFromAttributes(AttributeSet attrs)
	{
		...
		mWaveformColor = a.getColor(R.styleable.SimpleWaveformView_waveformColor, getResources().getColor(R.color.waveform_selected));
		mPlaybackIndicatorColor = a.getColor(R.styleable.SimpleWaveformView_indicatorColor, getResources().getColor(R.color.playback_indicator));
		int width = (int) ConversionUtils.dpToPx(getContext(), 4);
		mPlaybackIndicatorWidth = a.getDimensionPixelSize(R.styleable.SimpleWaveformView_indicatorWidth, width);
		a.recycle();
	}

	public void setAudioFile(SoundFile audioFile)
	{
		if (audioFile == null || audioFile.getNumFrames() == 0)
		{
			String text = audioFile == null ? "Audio null" : "Not enough frames " +
				audioFile.getNumFrames();
			Log.e(TAG, text);
			return;
		}

		Log.e(TAG, "Frames " + audioFile.getNumFrames());
	...
	public void setPlaybackPosition(int position)
	{
		...
		float frameDp = ConversionUtils.pxToDp(getContext(), frame);
		mPlaybackIndicatorPosition = (int) (frameDp / (float) mNumFrames * getMeasuredHeight());
		invalidate();
	}
	...
	private void computeIntsForThisZoomLevel()
	{
		...
		Log.e(TAG, "Width " + measuredWidth + " values count " + valuesCount);
